...
class SO3_BatchNorm(nn.Module):
    def __init__(self, type_list, eps=1e-5, momentum=0.1, affine=True, reduce='mean'):
        '''
        :param Rs: list of tuple (multiplicity, dimension)
        '''
        super().__init__()

        self.Rs = [(cur_type.mul,group_rep("SO3",cur_type)[-1]) for cur_type in type_list]
        self.eps = eps
        self.momentum = momentum
        self.affine = affine

        num_scalar = sum(cur_type.mul for cur_type in type_list if group_rep("SO3",cur_type)[-1] == 1)
        num_features = sum(cur_type.mul for cur_type in type_list)
