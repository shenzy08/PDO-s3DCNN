...
class Shrec17(torch.utils.data.Dataset):
    ...
    def __init__(self, root, mode, perturbed=True, download=False, transform=None, target_transform=None):
        ...
        if not mode in ["train", "test", "val","train_and_val"]:
...
#         print("self.dir:", self.dir)
    ...
    def _download(self, require_file):
        import requests

        file_path = os.path.join(self.root, require_file)

        if os.path.exists(file_path):
            return file_path

        print('Downloading ' + require_file)
    ...
    def download(self, dataset, perturbed):
        ...
        try:
            ...
            if e.errno == os.errno.EEXIST:
                ...
                raise
        require_file = self.root + "/" + dataset + ("_perturbed.zip" if perturbed else ".zip")
        self._unzip(require_file)
        self._fix()
